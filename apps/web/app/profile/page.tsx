  ...
  return (
    <main className="min-h-screen bg-base text-fg pb-24 page-enter">
      <nav className="sticky top-0 z-30 bg-base/80 backdrop-blur-md border-b border-edge-dim px-5 py-3 flex items-center justify-between">
        ...
        <h1 className="text-sm font-semibold text-fg-muted">Profile</h1>
        <div className="w-16" />
      </nav>
      ...
      <div className="max-w-lg mx-auto px-5 py-8 space-y-5 reveal">
        ...
        <div className="flex flex-col items-center py-6">
          ...
          {fullName && <p className="text-base font-semibold text-fg">{fullName}</p>}
          {username && <p className="text-sm text-fg-muted">@{username}</p>}
          <p className="text-fg-faint text-xs mt-1">{user?.email}</p>
        </div>

        {/* Profile */}
        <div className="bg-surface rounded-2xl p-5 border border-edge-dim">
          <h2 className="font-semibold text-fg mb-4">Edit profile</h2>
        ...
        <div className="bg-surface rounded-2xl p-5 border border-edge-dim">
          <div className="flex items-center justify-between">
            ...
            >
              <span className={`absolute left-0 top-1 w-5 h-5 rounded-full bg-white shadow-md transition-transform ${pushEnabled ? 'translate-x-6.5' : 'translate-x-1'}`} />
            </button>
          </div>
        </div>
